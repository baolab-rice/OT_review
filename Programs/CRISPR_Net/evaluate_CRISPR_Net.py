...
from Programs.Evaluation import encode_data
import glob
...
def CRISPR_Net_model():
    ...
    # tttt
    #blstm_out = Bidirectional(LSTM(15, return_sequences=True, name="LSTM_out"))(mixed)
...
## OT_Review
def CRISPR_Net_training(PATH, MODEL, DATA_X, DATA_y, batch_size=20000, epochs=100, callbacks = []):
    ...
    try:
        ...
        if os.path.isdir(model_path):
            ...
            os.mkdir(model_path)
        with open(model_path + MODEL, "w") as jason_file:
            jason_file.write(model_jason)
        model.save_weights(model_path + MODEL.replace(".json",".weights.h5"))
        print("Saved model to disk!")
    except:
        print("something wrong when saving model")
    return model

def CRISPR_Net_testing(weights_file, MODEL, DATA_X, DATA_y):
